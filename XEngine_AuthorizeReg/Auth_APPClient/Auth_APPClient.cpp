...
LPCTSTR lpszSerial = _T("123123");
LPCTSTR lpszEmail = _T("486179@qq.com");
__int64 nQQNumber = 486179;
__int64 nIDNumber = 511025111111111111;
		...
		if (XClient_TCPSelect_RecvMsg(m_Socket, tszMsgBuffer, &nMsgLen, TRUE))
		{
			...
			else if (XENGINE_COMMUNICATION_PROTOCOL_OPERATOR_CODE_AUTH_REPGETPASS == unProtoCode)
			{
				if (0 == nError)
				{
					...
					bPass = FALSE;
					printf(_T("找回密码成功,账号:%s,密码:%s"), st_AuthProtocol.tszUserName, st_AuthProtocol.tszPassword);
				}
				else
				{
					printf(_T("找回密码失败,错误:%d"), nError);
				}
			}
			else if (XENGINE_COMMUNICATION_PROTOCOL_OPERATOR_CODE_AUTH_REPGETTIME == unProtoCode)
			{
				if (0 == nError)
				{
					...
					bLeftTime = FALSE;
					printf(tszMsgBuffer, _T("获取剩余时间成功,账号:%s,地址:%s,剩余时间:%lld 分钟,在线时间:%lld,过期日期:%s,类型:%d"), st_AuthTime.tszUserName, st_AuthTime.tszUserAddr, st_AuthTime.nTimeLeft, st_AuthTime.nTimeONLine, st_AuthTime.tszLeftTime, st_AuthTime.enSerialType);
				}
				else
				{
					printf(_T("获取时间失败,错误:%d"), nError);
				}
			}
			else if (XENGINE_COMMUNICATION_PROTOCOL_OPERATOR_CODE_AUTH_NOTIFYMSG == unProtoCode)
			{
				if (0 == nError)
				{
					...
					printf(_T("读取到公告:%s"), tszMsgInfo);
				}
				else
				{
					printf(_T("获取数据失败,错误:%d"), nError);
				}
			}
...
int AuthClient_Register()
{
	...
	strcpy(st_UserTable.st_UserInfo.tszPassword, lpszPass);
...
int AuthClient_Login()
{
	...
	strcpy(st_AuthUser.tszPassword, lpszPass);
